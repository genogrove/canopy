...
- `score_max` is the rE2G confidence in `[0,1]` — **sort by it**; `n_rep` is replicate support.
  `byCohort` has one entry per declared cohort; report the cohort name with the score.
...
def enhancers_of(gene):                    # (enhancer node, evidence) pairs, strongest first
    pairs = [(e, m["byCohort"]) for e, m in g.get_edge_list(gene)
             if m and m["rel"] == "regulated_by"]
...
`TARGETS: [{"region": ...}]` and filters the variant's `intersect` on `source == "ENCODE-rE2G"`.
